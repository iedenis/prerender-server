      ...
      CDP.Version({ port: this.options.browserDebuggingPort })
        .then(info => {
          ...
          this.webSocketDebuggerURL =
            ...
            'ws://localhost:' + this.options.browserDebuggingPort + '/devtools/browser';
...
chrome.setUpEvents = async function (tab) {
  ...
  let bypassServiceWorker = !(
    this.options.enableServiceWorker == true || this.options.enableServiceWorker == 'true'
  );
...
chrome.getHarFile = function (tab) {
  return new Promise((resolve, reject) => {
    ...
    const firstRequest =
      tab.prerender.pageLoadInfo.entries[tab.prerender.pageLoadInfo.firstRequestId].requestParams;
    const wallTimeMs = firstRequest.wallTime * 1000;
    const startedDateTime = new Date(wallTimeMs).toISOString();
    const onContentLoad =
      tab.prerender.pageLoadInfo.domContentEventFiredMs - tab.prerender.pageLoadInfo.firstRequestMs;
